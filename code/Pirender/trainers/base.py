...
class BaseTrainer(object):
    ...
    def load_checkpoint(self, opt, which_iter=None, del_map = True):
        ...
        current_epoch, current_iteration = self._load_checkpoint(
            checkpoint_path, resume, del_map = del_map)
        return current_epoch, current_iteration

    def _load_checkpoint(self, checkpoint_path, resume=True, del_map = True): #TODO: fix
        print("load")
        ...
        if del_map:
            print("del_map")
            del checkpoint['net_G_ema']['mapping_net.first.0.bias']
            del checkpoint['net_G_ema']['mapping_net.first.0.weight']
            del checkpoint['net_G']['module.mapping_net.first.0.weight']
            del checkpoint['net_G']['module.mapping_net.first.0.bias']
            #Optional:
            for key in list(checkpoint['net_G_ema'].keys()):
                if 'mapping_net' in key:
                    del checkpoint['net_G_ema'][key]
            for key in list(checkpoint['net_G'].keys()):
                if 'mapping_net' in key:
                    del checkpoint['net_G'][key]
        if not ('net_G_ema' in checkpoint):
            self.net_G.load_state_dict(checkpoint['state_dict'], strict=False)
            self.net_G_ema.load_state_dict(checkpoint['state_dict'], strict=False)
        ...
        if self.opt.phase == 'train' and resume:
            ...
            if not del_map:
                ...
                print('optimizers and schdules are not loaded')
    ...
    def end_of_iteration(self, data, current_epoch, current_iteration):
        ...
        elif current_iteration % self.opt.image_save_iter == 0 or self.fst_iter_flag:
            self.fst_iter_flag = False
            self.save_image(self._get_save_path('image', 'jpg'), data)
