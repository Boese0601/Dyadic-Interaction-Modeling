...
class VQAutoEncoder(BaseModel):
    ...
    def decode_feats(self, quant):
        #BCL
        quant = quant.permute(0,2,1)
        quant = quant.view(quant.shape[0], -1, self.args.face_quan_num, self.args.zquant_dim).contiguous()
        quant = quant.view(quant.shape[0], -1,  self.args.face_quan_num*self.args.zquant_dim).contiguous()
        quant = quant.permute(0,2,1).contiguous()
        dec = self.decoder.forward_feats(quant) ## z' --> x
...
class VQSpeakerAutoEncoder(BaseModel):
    """ VQ-GAN model """

    def __init__(self, args):
        super().__init__()
        self.encoder = TransformerEncoder(args)
        self.decoder_v = TransformerDecoder(args, 56)
        self.decoder_a = TransformerDecoder(args, 768)
        self.quantize = VectorQuantizer(args.n_embed,
                                        args.zquant_dim,
                                        beta=0.25)
        self.args = args
    ...
    def decode(self, quant):
        ...
        quant = quant.view(quant.shape[0], -1,  self.args.face_quan_num*self.args.zquant_dim).contiguous()
        quant = quant.permute(0,2,1).contiguous()
        dec_a = self.decoder_a(quant) ## z' --> x
        dec_v = self.decoder_v(quant) ## z' --> x
        dec = torch.cat([dec_v, dec_a], dim=-1)
        return dec
